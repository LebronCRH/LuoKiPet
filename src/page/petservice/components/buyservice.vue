...
<div class="zhe2" @click="modalTaggle()"></div>
   ...
   <div class="Top">
     ...
     <div class="Service_Edit">
      <div class="AddBagActive" v-show="HasselectPack" @click="AddPackageCart(ShopId,ShopName,ServiceId,currentservice.serviceName,selectPackId,selectPackage.name,BuyNum,selectPackage.sprice || ServicePackage[0].sprice,selectPackage.oldprice || ServicePackage[0].oldprice)">加入券包</div>
      <div class="AddBag" v-show="!HasselectPack">加入券包</div>
      <router-link v-show="HasselectPack" :to="{ path: 'confirmorder',query:{ServiceId:currentservice.ForServiceID,Index:selectPackId,BuyNum:BuyNum},append:true}">
