...
<div ref="HelperLine"></div>
  ...
  <div class="BigIput">
    ...
    <div class="UserNameNo MessageItem" v-show="UserPhoneHas==2">
      <span class="No">该手机号已被使用</span>
...
<!--     <router-link :to="'/createuserthirdly'"> -->
...
<script>
  ...
  import {UserRegisterPhoneHas} from '../../service/getdata'
	export default{
		data(){
			return{
        UserPhone:'',
    ...
    methods:{
      NextPage(){
        if(this.UserPhone!="")
          {
            // axios.get("user/UserRegisterPhoneHas",{params:{"Phone":this.UserPhone}}).then((response)=>{  
            UserRegisterPhoneHas(this.UserPhone).then(response=>{
              if(response==0){
                this.UserPhoneHas=2;
